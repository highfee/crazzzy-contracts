...
contract CrazzzyMonstersStaking is Ownable, IERC721Receiver, ERC721Holder {
    ...
    uint256 public rewardTime = 60; // Default reward time in seconds (adjustable)
    ...
    event RewardTokenChanged(address indexed newRewardToken);
    event RewardTimeChanged(uint256 newRewardTime);
    ...
    function unstakeAllNFTs() public {
        ...
        require(stakedNFTsArray.length > 1, "You don't have any staked NFTs");
    ...
    function claimReward(address collection, uint256 tokenId) public {
        ...
        stakingInfo.accumulatedReward = 0; // Reset accumulated reward to 0 after claiming
    ...
    // Claim rewards for all NFTs
    function claimAllRewards() public {
        StakedNFT[] storage stakedNFTsArray = userStakedNFTs[msg.sender];
        for (uint256 i = 0; i < stakedNFTsArray.length; i++) {
            claimReward(
                stakedNFTsArray[i].collection,
                stakedNFTsArray[i].tokenId
    ...
    function getTotalRewards(
        ...
        StakedNFT[] memory stakedNFTsArray = userStakedNFTs[owner];
    ...
    function _calculateReward(
        ...
        return (timeSinceLastClaim * rewardRate) / rewardTime;
    }
